...
for i in range(len(list)):
    time.append(
        timeit.timeit("insertion_sort({})".format(list[i]), setup="from __main__ import insertion_sort", number=1))
    count.append(insertion_sort(list[i]))
    time2.append(
        timeit.timeit("insertion_sort({})".format(list2[i]), setup="from __main__ import insertion_sort", number=1))
    count2.append(insertion_sort(list2[i]))
